...
export default function Footer() {
  return (
    <footer className="bg-brand-black border-t border-white/5 pt-16 pb-8 px-8 md:px-12">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-10 md:gap-12 mb-14">
          ...
          <div className="col-span-2 md:col-span-1">
            <Link
              href="/"
              className="font-chillax font-bold text-2xl tracking-[4px] text-brand-yellow block mb-4"
            >
              CHEEZIOUS
            </Link>
          ...
          {FOOTER_COLS.map((col) => (
            <div key={col.heading}>
              <h4 className="text-[0.65rem] tracking-[3px] uppercase text-brand-yellow font-bold mb-5">
                {col.heading}
              </h4>
        ...
        {/* Bottom bar */}
        <div className="border-t border-white/5 pt-7 flex flex-col md:flex-row items-center justify-between gap-5">
          ...
          <div className="flex gap-3">
            {SOCIAL_LINKS.map((s) => (
              <Link
                ...
                href={s.href}
                aria-label={s.label}
                className="w-9 h-9 rounded-full border border-white/10 flex items-center justify-center text-[0.7rem] font-bold text-brand-gray hover:border-brand-yellow hover:text-brand-yellow hover:bg-brand-yellow/8 transition-all duration-200"
              >
                {s.label}
              </Link>
            ))}
